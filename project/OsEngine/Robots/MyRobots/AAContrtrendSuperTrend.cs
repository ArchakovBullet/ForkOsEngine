...
namespace OsEngine.Robots.MyRobots
{
    ...
    public class AAContrtrendSuperTrend : BotPanel
    {
        ...
        public AAContrtrendSuperTrend(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            _SP.DataSeries[2].Color = Color.BlueViolet;
            _SP.Save();


            // Subscribe to the indicator update event
            ParametrsChangeByUser += AAContrtrendSuperTrend_ParametrsChangeByUser;

            // Subscribe to the candle finished event
            _tab.CandleFinishedEvent += _tab_CandleFinishedEvent;

			//  При включенном этом событии робот даже не приходит в метод  private void _tab_CandleUpdateEvent(List<Candle> candles)
		...
		private void AAContrtrendSuperTrend_ParametrsChangeByUser()
        {
            ((IndicatorParameterInt)_SP.Parameters[0]).ValueInt = _lengthSP.ValueInt;
            ((IndicatorParameterDecimal)_SP.Parameters[1]).ValueDecimal = _spDeviation.ValueDecimal;
            ((IndicatorParameterString)_SP.Parameters[2]).ValueString = _typePrice.ValueString;
        ...
        private void _tab_CandleFinishedEvent(List<Candle> candles)
        {
            ...
            if (candles.Count < _lengthSP.ValueInt + 10 || candles.Count < _lengthFI.ValueInt)
            {
                return;
            }
        ...
        private void LogicOpenPosition(List<Candle> candles)
        {
			...
			if (openPositions == null || openPositions.Count == 0)
            {
                ...
                {
                    if (lastPrice > lastSp)//&&    &&      && _lastFI > _prevFI  && _lastFI > 0 && _prevFI < 0
					{
